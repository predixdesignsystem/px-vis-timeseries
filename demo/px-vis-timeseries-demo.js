...
import '../px-vis-timeseries.js';
import { Polymer } from '@polymer/polymer/lib/legacy/polymer-fn.js';
import { html } from '@polymer/polymer/lib/utils/html-tag.js';
Polymer({
  _template: html`
    <!-- Header -->
    <px-demo-header module-name="px-vis-timeseries" description="	The	px-vis-timeseries component creates a timeseries chart. With timeseries, you can create line and scatter timeseries charts complete with user interaction, events, thresholds, and a navigator. The chart is built off px-vis components which utilize d3.js 4.0+." desktop="">
    </px-demo-header>
    ...
    <px-demo-interactive>
      ...
      <px-demo-component slot="px-demo-component" class="flex__item flex" style="width:100%">
        <div class="flex--column flex__item u-mt+" style="width:100%;">
        <px-vis-timeseries id="chart" prevent-resize="" chart-data="{{props.chartData.value}}" include-all-series="{{props.includeAllSeries.value}}" width="{{props.width.value}}" height="{{props.height.value}}" chart-horizontal-alignment="{{props.chartHorizontalAlignment.value}}" chart-vertical-alignment="{{props.chartVerticalAlignment.value}}" margin="{{props.margin.value}}" x-axis-config="{{props.xAxisConfig.value}}" y-axis-config="{{props.yAxisConfig.value}}" register-config="{{props.registerConfig.value}}" tooltip-config="{{props.tooltipConfig.value}}" show-tooltip="{{props.showTooltip.value}}" hide-register="{{props.hideRegister.value}}" hide-gridlines-x="{{props.hideGridlinesX.value}}" hide-gridlines-y="{{props.hideGridlinesY.value}}" series-config="{{props.seriesConfig.value}}" dynamic-menu-config="{{props.dynamicMenuConfig.value}}" event-config="{{props.eventConfig.value}}" event-data="{{props.eventData.value}}" threshold-config="{{props.thresholdConfig.value}}" threshold-data="{{props.thresholdData.value}}" display-threshold-title="{{props.displayThresholdTitle.value}}" chart-extents="{{props.chartExtents.value}}" toolbar-config="{{props.toolbarConfig.value}}" match-ticks="{{props.matchTicks.value}}" selection-type="{{props.selectionType.value}}" render-to-canvas="{{props.renderToCanvas.value}}" debounce-resize-timing="{{props.debounceResizeTiming.value}}">
        </px-vis-timeseries>
      </div>
      </px-demo-component>
      <!-- END Component ------------------------------------------------------>

      <px-demo-component-snippet slot="px-demo-component-snippet" element-properties="{{props}}" element-name="px-vis-timeseries" codepen-link="https://glitch.com/edit/#!/px-vis-timeseries-demo" suppress-property-values="[[suppressPropertyValues]]">
      </px-demo-component-snippet>
    </px-demo-interactive>

    <!-- API Viewer -->
    <px-demo-api-viewer source="px-vis-timeseries" hide="[[apiHide]]" mark-private="[[apiMarkPrivate]]" mark-read-only="[[apliMarkReadOnly]]" change-description="[[apiChangeDescription]]">
    </px-demo-api-viewer>
...
`,
  ...
  demoProps: {
    ...
    seriesConfig: {
      type: Object,
      defaultValue: {"y0":
      {"name":"y0","x":"timeStamp","y":"y0","yAxisUnit":"F","axis":{"id":"axis1","side":"left","number":"1"}},
      "y1":{"name":"y1","x":"timeStamp","y":"y1","yAxisUnit":"Hz","axis":{"id":"axis2","side":"right","number":"1"}},
      "y2":{"name":"y2","x":"timeStamp","y":"y2","yAxisUnit":"C","axis":{"id":"axis3","side":"left","number":"2"}},
      "y3":{"name":"y3","x":"timeStamp","y":"y3","yAxisUnit":"F","axis":{"id":"axis4","side":"right","number":"2"}}},
      inputType: 'code:JSON'
    },
    ...
    eventData: {
      type: Array,
      defaultValue: [{"id":"123","time":1328714240000,"label":"Recalibrate"},
          {"id":"456","time":1347640960000,"label":"Fan start"},
          {"id":"789","time":1378125240000,"label":"Fan stop"},
